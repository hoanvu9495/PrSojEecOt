    ...
    public class QuanLyDanhHieuCaNhanController : BaseController
    {
        ...
        public PartialViewResult FindDanhHieu(FormCollection coll)
        {
            ...
            if(!string.IsNullOrEmpty(coll["YEAR"])){
                DanhHieus = DanhHieus.Where(x => x.YEAR).ToList();
            }
            if (!string.IsNullOrEmpty(coll["DANHHIEUTHIDUA"]))
            {
                var TenDanhHieu = coll["DANHHIEUTHIDUA"].Trim();
                DanhHieus = DanhHieus.Where(x => Ultilities.RemoveSign4VietnameseString(x.DANHHIEUTHIDUA.ToLower()).Contains())
            }
            ViewData["Search"] = "1";
