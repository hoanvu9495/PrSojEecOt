...
using Web.Areas.QLHopDongLaoDongArea.Models;
using Web.Common;
using Web.Custom;
using Web.FwCore;

namespace Web.Areas.QLHopDongLaoDongArea.Controllers
{
    public class QLHopDongLaoDongController : BaseController
    {
        //
        ...
        public ActionResult Index()
        {
            ...
            if (!Ultilities.IsInActivities(user.ListThaoTac, "/QLHopDongLaoDongArea/QLHopDongLaoDong/Index"))
            {
                return RedirectToAction("UnAuthor", "Home", new { area = "" });
            }
            HscbHopDongLD68Business = Get<HscbHopDongLD68Business>();

            SessionManager.Remove("ListHopDongLaoDong");
            SessionManager.Remove("ListHopDongLaoDongSearch");

            List<HscbHopDongLaoDongBO> lstHopDong = FillDataToGrid();

            SessionManager.SetValue("ListHopDongLaoDong", lstHopDong);
            ViewData["Search"] = "0";
            return View();
        }
        public List<HscbHopDongLaoDongBO> FillDataToGrid()
        {
            UserInfoBO user = (UserInfoBO)SessionManager.GetUserInfo();
            HscbHopDongLD68Business = Get<HscbHopDongLD68Business>();
            List<HscbHopDongLaoDongBO> lstHopDong = HscbHopDongLD68Business.GetList(user.DonViID);
            return lstHopDong;
        }
        ...
        {
            HscbHopDongLD68Business = Get<HscbHopDongLD68Business>();
            List<HscbHopDongLaoDongBO> HopDongResult = (List<HscbHopDongLaoDongBO>)SessionManager.GetValue("ListHopDongLaoDong");
            ...
            SessionManager.SetValue("ListHopDongLaoDongSearch", HopDongResult);
            return PartialView("_QLHopDongLaoDongSearchResult");
        }

        public ActionResult CreateHopDongLaoDong()
        {
            return View();
        }
        public ActionResult EditHopDongLaoDong(int id)
        {
            UserInfoBO user = (UserInfoBO)SessionManager.GetUserInfo();
            if (!Ultilities.IsInActivities(user.ListThaoTac, "/QLHopDongLaoDongArea/QLHopDongLaoDong/Index"))
            {
            ...
            List<HscbHopDongLaoDongBO> HoSoResult = (List<HscbHopDongLaoDongBO>)SessionManager.GetValue("ListHopDongLaoDong");
            if (HoSoResult == null)
            {
            ...
            HopDongLaoDongViewModel model = new HopDongLaoDongViewModel();
        ...
        public JsonResult Save(HSCB_HOPDONG_LD_68 HopDong, FormCollection fm)
        {
            ...
            UserInfoBO user = (UserInfoBO)SessionManager.GetUserInfo();
            HopDong.LOAI_HOP_DONG = QLhopDongConstant.HOPDONG_LAODONG;
        ...
        {
            HscbHopDongLD68Business = Get<HscbHopDongLD68Business>();
            HopDongLaoDongDetailModel model = new HopDongLaoDongDetailModel();
